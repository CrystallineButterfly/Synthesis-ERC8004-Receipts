...
MODULE = importlib.import_module('agents.receipts_mesh')
...
class ProjectContextTest(unittest.TestCase):
    ...
    def test_primary_names_match(self) -> None:
        spec = MODULE.build_project_spec()
        self.assertEqual(spec.primary_python_module, 'receipts_mesh')
        self.assertEqual(spec.primary_contract_name, 'ReceiptsMeshRegistry')
