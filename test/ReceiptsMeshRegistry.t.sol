...
contract ReceiptsMeshRegistryTest is Test {
    ReceiptsMeshRegistry private controller;
    ...
    bytes32 private actionId = keccak256("synthesis_erc8004_receipts");
    bytes32 private subjectId = keccak256("subject");

    function setUp() public {
        controller = new ReceiptsMeshRegistry(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
    ...
    function testExecuteBoundedActionRevertsWithoutReportedBalance() public {
        ReceiptsMeshRegistry freshController = new ReceiptsMeshRegistry(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
